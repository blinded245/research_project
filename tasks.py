...
def clean(ctx, docs=True, bytecode=True, builds=True):
    ...
    if builds:
        ctx.run('{} setup.py clean'.format(sys.executable))
    ...
    if bytecode:
        folders.append('src/research_project/__pycache__')

    if builds:
        folders.append('build/')
        folders.append('src/research_project.egg-info/')
...
def docs(ctx, rebuild=True, check_links=False):
    ...
    if rebuild:
        clean(ctx)
    ctx.run('{} -m sphinx -b doctest docs dist/docs'.format(sys.executable))
    ctx.run('{} -m sphinx -b html docs dist/docs'.format(sys.executable))
    if check_links:
        ctx.run('{} -m sphinx -b linkcheck docs dist/docs'.format(sys.executable))
...
def check(ctx):
    ...
    ctx.run('{} setup.py check --strict --metadata --restructuredtext'.format(sys.executable))
...
def release(ctx, release_type):
    ...
    ctx.run('{} setup.py clean --all sdist bdist_wheel'.format(sys.executable))
